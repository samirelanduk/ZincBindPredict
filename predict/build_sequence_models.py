...
for csv in csvs:
    ...
    data = pd.read_csv(f"data/csv/sequence/{csv}.csv").values
    np.save(f"predict/npy/sequence/{csv}", data)

    # Split into train and test data, and save
    data_train, data_test = train_test_split(data)
    data_train, data_test = data_train[:, 1:], data_test[:, 1:]
    np.save(f"predict/npy/sequence/{csv}_train", data_train)
    ...
    for Model, hyperparameters in SEQUENCE_CONFIG.items():
        ...
        # Save model
        joblib.dump(model, f"predict/models/sequence/{csv}-{Model.__name__}.joblib")