...
for category in categories:
    ...
    for family in families:
        ...
        hp_space = {"criterion": ["gini"], "n_estimators": [100], "max_depth": [None]}
        ...
        plt.savefig(f"predict/models/{category}/{family}-randomforest-learning-curve.svg")
        plt.clf()
        ...
        plt.title(f"{family}, randomforest")
        plt.legend()
        plt.savefig(f"predict/models/{category}/{family}-randomforest-auc.svg")
        plt.clf()
        ...
        model._test_recall = results[category][family]["recall"] = test_recall
        model._test_precision = results[category][family]["precision"] = test_precision
...
for category in results:
    ...
    for family in results[category]:
        print("   ", family)
        for metric in results[category][family]:
            print(f"      {metric}: {round(results[category][family][metric], 3)}")